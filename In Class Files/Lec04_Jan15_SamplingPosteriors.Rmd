...
posterior_table <- tibble(p_w_proposed=seq(from=0, to=1, by=0.001))%>%
...
test = tibble(diff = posterior_table1$posterior - posterior_table2$posterior)
...
ggplot(data=posterior_table2, aes(x=p_w_proposed, y=posterior))+
  geom_line(color="red") + 
  geom_line(aes(x=p_w_proposed, y=prior/1000) , color="blue")
```
