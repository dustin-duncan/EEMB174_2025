...
Pr(\mathrm{y_i}) = \prod PDF(\mathrm{Normal}(y_i , \mu , \sigma))
...
height_lik_f <- function(mu_input,sigma_input){
  # Adds up the log likelihood values, which is equivalent to multiplying the raw likelihood values for each observation of height
  sum(dnorm(
  d2$height , 
  mean=mu_input,
  sd=sigma_input,
