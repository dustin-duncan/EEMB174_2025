...
*When is this lab due?* Labs are due on the Thursday after they are assigned. However, in many cases you can complete them during the lab period itself. This assignment is due on Thursday, 1/16/2024. You must answer all the problems, which are written in **bold**. 
It is not equal to one because the pool size is one, and the way we have set up the function is if a test returns positive in a pool, the entire pool is retested. So the negative pools generate one test and the positive pools generate 2 tests (in this case) leading to greater than one test per person
Sign returns either 1, 0, or -1 depending on the sign of the target column
The numbers change each time we run it because it is taking a random sample each time it is run 
...
pools <- mutate(pools, 
                retest = sign(positive), # Assigns 1 if pos, 0 if zero
                tests = 1 + retest * k)  # Says "all have at least 1 test, if retest is 1 then multiply that by the pool size 'k' and add the first test to it. If retest is zero then it is just the first test

# Take mean number of tests needed
mean(pools$tests) / k # Because you're saying this is the mean number of tests per pool, dividing by k (# of people in pool) gives you the mean number of tests per person
```
...
*<Assigns 1 if pos, 0 if zero, and -1 if neg>* 
...
We can chain pipes together. The code below takes an imaginary number, then takes the real part of it, then takes the absolute value, then takes the base 10 log, then finally prints it.
...
```{r run_function}
calc_tests(1)  # Mean tests needed for when pools have three people each
```
...
*<The numbers change each time we run it because every time we run it it's taking a new sample from a binomial distribution even if k stays the same. It is not equal to one because even though the pool size is one, each time the first test result gives a positive we retest that group, adding 1 +   1 x k to the total that we're taking the mean of.>*
...
keffects <- keffects %>%
  rowwise() %>% # allows you to compute on a data frame one row at a time
...
*Yes, five is the best pool size.* 

#### Problem 7 (Bonus)

Modify the above code to try up to pool size 100. What do you see? (This will take a few minutes to run!)

*We see that the average number of tests drops to its minimum at a pool size of 5, then increases as we increase the pool size up to about 75 where it levels off around just greater than 1 test per person in the pool.*

```{r}
keffects <- tibble(k = rep(seq(100), 100)) # Sequence up to 100, 100 times

keffects <- keffects %>%
  rowwise() %>% # allows you to compute on a data frame one row at a time
  mutate(ave_tests = calc_tests(k), poolsize = as.integer(k))  # convert *k* to an integer to get geom_boxplot to work properly. 

ggplot(keffects, aes(x = k, y = ave_tests, group = poolsize)) + 
  geom_boxplot()
```
